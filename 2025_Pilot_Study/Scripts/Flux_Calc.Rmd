...
```{r}
raw7810 <- read_tsv("../Raw_Licor_Output_By_Day/TG10-01532-2025-05-31T090000_Until_3Sep2025.data",
...
flux_over_time <- ggplot(final_flux_graphing, aes(x = Date, y = CH4_flux_g_ha_day,  colour =  Treatment))+
  ...
  geom_text(aes(x = as.POSIXct("2025-06-26"), 
                y = max(CH4_flux_g_ha_day, na.rm = TRUE) -2500,
  ...
  geom_text(aes(x = as.POSIXct("2025-08-04"), 
                y = max(CH4_flux_g_ha_day, na.rm = TRUE) -2500,
                label = "1st defoliation (10 cm trim - approx)"),
            angle = 90, vjust = -0.5, hjust = 0, color = "darkgreen", size =6) +
  # Second defoliation     
  geom_vline(xintercept = as.POSIXct(c("2025-08-18")), 
             linetype = "dashed", color = "darkgreen") +
  geom_text(aes(x = as.POSIXct("2025-08-18"), 
                y = max(CH4_flux_g_ha_day, na.rm = TRUE) -2500,
                label = "2nd defoliation (to top of leaf shealth)"),
            angle = 90, vjust = -0.5, hjust = 0, color = "darkgreen", size =6) 


flux_over_time

ggsave(flux_over_time, filename = "../Figures/flux_over_time_rapidC.jpg", height = 25, width = 25, units = "cm", dpi=400)

ggplot(final_flux, aes(x = Date, y = CH4_flux_g_ha_day,  colour =  Treatment))+
  geom_point()
...
# Some prelimary analysis

So it seems that if we throw everything into a linear model no effect lol - which make sense.

```{r}
final_flux$Date_Factor <- as.factor(final_flux$Date)

model <- lm(CH4_flux_g_ha_day~Treatment*Date_Factor, data= final_flux)
...
str(final_flux)
Treatment_means = emmeans(model,spec = 'Treatment',by = 'Date_Factor')
Treatment_effects = contrast(Treatment_means, method = 'pairwise', adjust = "Tukey")
summary(Treatment_effects)

cld(Treatment_means, alpha = 0.05)
```

Now let's try to just subset out the last date? So if it is a T-test, there are differences!

```{r}
final_flux$Date_Factor <- as.factor(final_flux$Date)

flux_defoliated <- final_flux %>% filter(as.Date(Date) == as.Date("2025-08-25"))

model <- lm(CH4_flux_g_ha_day~Treatment, data= flux_defoliated)
...
t.test(CH4_flux_g_ha_day ~ Treatment, data = flux_defoliated, var.equal = TRUE)
```
